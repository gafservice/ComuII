%% Comparación de Pulsos Coseno Alzado - Tiempo y Frecuencia (Root Raised Cosine)
clc; clear; close all;
...
for k = 1:length(roll_offs)
    beta = roll_offs(k);
    pt = rcosdesign(beta, span, L, 'sqrt');         % Pulso raíz de coseno alzado
...
ylabel('|P(f)|')
title('Espectro de Pulsos Root Raised Cosine')
